          ...
          <v-row justify="center" class="mt-5">
            <v-col cols="10" id="people-image">
              <div>
                ...
                <div style="text-align: center">You must assess your own health condition.</div>
...
export default {
  ...
  head() {
    return {
      title: 'Verify Health Assessment'
    }
  },
  data: () => ({
    valid: true,
  ...
  methods: {
    ...
    getPolicyId(policies) {
      if (!policies) return
      const policy = policies.find((policy) => policy.name === 'Proof of Self Check' && policy.version === '1.1')
      this.policy = policy
